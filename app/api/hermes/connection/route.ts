...
// The session cookie is stored server-side and never returned to the browser.
...
export async function PUT(req: NextRequest) {
  ...
  const next: HermesConnection = {
    ...
    // Blank/undefined token means "keep the existing cookie". The login
    // endpoint is the canonical way to set a new cookie — the UI should not
    // be sending tokens here.
    token:
    ...
    chatKey:
      incoming.chatKey === "" || incoming.chatKey === undefined
        ? current.chatKey
        : incoming.chatKey,
    // Undefined keeps the existing list; an explicit object replaces it.
